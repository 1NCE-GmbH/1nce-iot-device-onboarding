...
curl https://replace_with_code_bucket_name.s3.replace_with_code_bucket_region_name.amazonaws.com/replace_with_version/vpnConfig/$breakout_region.conf -o /etc/openvpn/openvpn-1nce-client.conf
...
aws ssm put-parameter --name replace_with_ssm_param_name_to_proxy_server --value "$ip:replace_with_nginx_port" --type String --overwrite
# Get and export Nginx config as env variables
export ONBOARDING_ENDPOINT=$(aws ssm get-parameter --name replace_with_ssm_param_name_to_onboarding_endpoint | jq -r '.Parameter.Value')
